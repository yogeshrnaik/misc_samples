...
public class Lesson3 {
    ...
    /**
     * Computes the Levenshtein distance between every pair of words in the subset, and returns a matrix of distances. This actually
     * computes twice as much as it needs to, since for every word a, b it should be the case that lev(a,b) == lev(b,a) i.e., Levenshtein
    ...
    static int[][] computeLevenshtein(List<String> wordList, boolean parallel) {
        ...
        // YOUR CODE HERE
        IntStream itr1 = getIntStream(LIST_SIZE, parallel);
        itr1.forEach(i -> {
            IntStream itr2 = getIntStream(LIST_SIZE, parallel);
            itr2.forEach(j -> distances[i][j] = Levenshtein.lev(wordList.get(i), wordList.get(j)));
        });

        return distances;
    }

    private static IntStream getIntStream(final int size, boolean parallel) {
        IntStream itr = IntStream.range(0, size);
        return parallel ? itr.parallel() : itr;
    }
    ...
    static List<String> processWords(List<String> wordList, boolean parallel) {
        // YOUR CODE HERE
        return getStream(wordList, parallel).sorted()
    ...
    private static Stream<String> getStream(List<String> wordList, boolean parallel) {
        Stream<String> s = wordList.stream();
